...
function chordtoolTip(d,i){
            return "<h4>"+"</h4><table>"+
              "<span style='color:red'>" + cities[i].name + "</span>"+
              "<tr><td>Movein:</td><td>"+(cities[i].Inflow)+"</td></tr>"+
              "<tr><td>Moveout:</td><td>"+(cities[i].Outflow)+"</td></tr>"+
              "<tr><td>NetFlow:</td><td>"+(cities[i].Netflow)+"</td></tr>"+
              "</table>";
};
function mouseOverchord(d,i){
      d3.select("#tooltip").transition().duration(200).style("opacity", .9);
      d3.select("#tooltip").html(chordtoolTip(d,i))
...
.on('mouseover', function(d, i) { return mouseOverchord(d,i);})
